...
def PrimToCons(fP, fU, fMi):
    ...
    fU[3, :] = 0.5 * phy_const.m_e * fP[1, :] * fP[5, :] ** 2 + 3.0 / 2.0 * fP[1, :] * phy_const.e * fP[3,
                                                                                                     :]  # (rhoeUe_y^2 +  3/2*ni*e*Te)
...
def ConsToPrim(fU, fP, fMi, fA0, fJ=0.0):
    ...
    fP[3, :] = 2.0 / 3.0 * (fU[3, :] - 0.5 * fU[4, :] ** 2 / (phy_const.m_e * fU[1, :] / fMi)) / (
                phy_const.e * fP[1, :])  # Te
...
def InviscidFlux(fP, fF, fVG, fMi, tau_xy=0., heat_flux_vec=0.):
    ...
    fF[3, :] = (5.0 / 2.0 * fP[1, :] * phy_const.e * fP[3, :] + 0.5 * phy_const.m_e * fP[1, :] * fP[5, :] ** 2) * fP[4,
                                                                                                                  :] + tau_xy * fP[
                                                                                                                                5,
                                                                                                                                :] + heat_flux_vec  # (1/2*rhoe*uey^2*v_e + 5/2n_i*e*T_e*v_e)
...
def Source(fP, fS, fBarr, fisSourceImposed, fenableIonColl, wall_inter_type: str, fx_center, fimposed_Siz, fESTAR, fMi,
    ...
    if not fisSourceImposed:
        Kiz = 1.8e-13 * (((1.5 * Te) / Eion) ** 0.25) * np.exp(
            - 4 * Eion / (3 * Te))  # Ion - neutral  collision rate          MARTIN: Change
        Siz_arr = ng * ni * Kiz
    else:
...
def heatFluxImplicit(fP, fBarr, wall_inter_type: str, fx_center, fESTAR, fMi, fR1, fR2, fLTHR, fKEL, falpha_B, fDelta_x,
    ...
    b_mainDiag = (
            ...
            Coefficient[1:-1] * (kappa_12[:-1] * (fx_center[2:] - fx_center[1:-1]) + kappa_12[1:] * (
                fx_center[1:-1] - fx_center[:-2])) / (
                        (fx_center[1:-1] - fx_center[:-2]) * (fx_center[2:] - fx_center[1:-1]))
    )
...
def SetInlet(fP_In, fU_ghost, fP_ghost, fMi, fisSourceImposed, fMDOT, fA0, fVG, fTe_cath, fJ=0.0, moment=1):
    ...
    U_ey_In = fP_In[5]
    Energy_Ghost = 0.5 * phy_const.m_e * fP_In[1] * fP_In[5] ** 2 + 3.0 / 2.0 * fP_In[
        1] * phy_const.e * fTe_cath  # (2*fTe_cath - fP_In[3])
    ...
    fP_ghost[3] = 2.0 / 3.0 * (fU_ghost[3] - 0.5 * fU_ghost[4] ** 2 / (phy_const.m_e * fU_ghost[1] / fMi)) / (
                phy_const.e * fP_ghost[1])  # Te
...
def main(fconfigfile):
    ...
    if len(list_of_res_files) > 0:
        print(
            "Warning: all MacroscopicVars_<i>.pkl files in the " + Resultsdir + " location were deleted to welcome new data.")
    ...
    print(
        f"Checking mu value [m^2 s^-1 V^-1]. Bmax = {msp.BMAX * 10000:.0f}\talpha_B1 = {alpha_B1:.4e}\talpha_B2 = {alpha_B2:.4e}")
    # print(mu_eff_arr)
        ...
        while time < TIMEFINAL:
            ...
            if HEATFLUX and not IMPlICIT:
                dt_HF = heatFlux(np.concatenate([P_Inlet, P, P_Outlet], axis=1), S,
                                 ...
                                 ESTAR, Mi, R1, R2, LTHR, KEL, np.concatenate([[alpha_B[0]], alpha_B, [alpha_B[-1]]]),
                                 Delta_x)
                Delta_t = min(dt_HF, Delta_t)
            if HEATFLUX and IMPlICIT:
                dt_HF = Delta_t
                Te = heatFluxImplicit(np.concatenate([P_Inlet, P, P_Outlet], axis=1),
                                      np.concatenate([[Barr[0]], Barr, [Barr[-1]]]), wall_inter_type, x_center_extended,
                                      ESTAR, Mi, R1, R2, LTHR, KEL,
                                      np.concatenate([[alpha_B[0]], alpha_B, [alpha_B[-1]]]), Delta_x_extended, Delta_t)
                print(Te)
    ...
    if TIMESCHEME == "TVDRK3":
        ...
        while time < TIMEFINAL:
            ...
            J_1 = compute_I(P, V, time, Barr, wall_inter_type, x_center, ESTAR, Mi, R1, R2, LTHR, KEL, alpha_B, Delta_x,
                            A0, Rext, Delta_t)
            ...
            if HEATFLUX and IMPlICIT:
                dt_HF = Delta_t
                P[3, :] = heatFluxImplicit(np.concatenate([P_Inlet, P, P_Outlet], axis=1),
                                           np.concatenate([[Barr[0]], Barr, [Barr[-1]]]), wall_inter_type,
                                           x_center_extended, ESTAR, Mi, R1, R2, LTHR, KEL,
                                           np.concatenate([[alpha_B[0]], alpha_B, [alpha_B[-1]]]), Delta_x_extended,
                                           Delta_t)
