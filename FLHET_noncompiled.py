...
if MagneticFieldConfig["Type"] == "Default":
    ...
    saveBField = bool(
        config.getboolean(
            "Magnetic field configuration", "Save B-field", fallback=False
        )
    )
...
if MagneticFieldConfig["Type"] == "Default":
    ...
    B0 = Bmax * np.exp(
        -CmagIn * (((x_center - L0) / L0) ** 2.0)
    )  # Magnetic field within the thruster
    B0 = np.where(
        x_center < L0, B0, Bmax * np.exp(-CmagOut * (((x_center - L0) / L0) ** 2.0))
    )  # Magnetic field outside the thruster
alpha_B = (
...
if saveBField:
    ...
    plt.plot([L0 * 100, L0 * 100], [0.0, 1.2 * max(B0) * 1e4], "k--")
    plt.xlabel("x [cm]")
    plt.ylabel("B [G]")
...
def compute_I(P, V):
    ...
    Kel = compute_Kel(
        Te
    )  # Electron - neutral  collision rate     TODO: Replace by good one
