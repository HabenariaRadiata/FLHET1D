...
BASE_CONFIG="my_config_modifRLC_3p5.ini"
...
run_instance() {
    local voltage=$1
    local temp_config="temp3p5mgs_modifRLC_${voltage}V.ini"

    echo "Starting voltage ${voltage}V"

    # Create modified .ini
    sed -e "s/^Voltage.*/Voltage             = ${voltage}/" \
        -e "s|^Result dir.*|Result dir        = ./Results/voltage_sweep3p5mgs_modifRLC/test_${voltage}V|" \
