...
BASE_CONFIG="my_config_modifRLC.ini"
...
run_instance() {
    local voltage=$1
    local temp_config="temp2p5mgs_modifRLC_${voltage}V.ini"

    echo "Starting voltage ${voltage}V"

    # Create modified .ini
    sed -e "s/^Voltage.*/Voltage             = ${voltage}/" \
        -e "s|^Result dir.*|Result dir        = ./Results/voltage_sweep2p5mgs_modifRLC/test_${voltage}V|" \
